...
def parser(path, opath=None):
    ...
    for record in parser.records_json():
        ...
        if event["Event"]["System"]["EventID"] == 1:
            ...
            try:
                if event["Event"]["EventData"]["Image"] not in files_inserted:
                    ...
                    query_file = 'INSERT INTO public."Files" ("Filename","FileVersion","Description","Product","Company"' \
                                 ...
                                 "VALUES ('{}','{}','{}','{}','{}' {});".format(
                                    key,
                                    event["Event"]["EventData"]["Image"], event["Event"]["EventData"]["FileVersion"],
            ...
            try:
                query_pprocess = 'INSERT INTO public."Processes" ("ProcessGuid","ProcessId","Image") ' \
                                 "VALUES ('{}',{},'{}') ON CONFLICT DO NOTHING;".format(
                                                                event["Event"]["EventData"]["ParentProcessGuid"],
        ...
        if event["Event"]["System"]["EventID"] == 7:
            ...
            if event["Event"]["EventData"]["ImageLoaded"]:
                event["Event"]["EventData"]["ImageLoaded"] = str(
                    event["Event"]["EventData"]["ImageLoaded"]).replace("'", "\"")
        ...
        if event["Event"]["System"]["EventID"] == 8:
            try:  # Source Process
                query_sprocess = 'INSERT INTO public."Processes" ("ProcessGuid","ProcessId","Image")' \
                    ...
                    .format(event["Event"]["EventData"]["SourceProcessGuid"],
                            event["Event"]["EventData"]["SourceProcessId"],
                            event["Event"]["EventData"]["SourceImage"])
            ...
            try:  # Target Process
                query_tprocess = 'INSERT INTO public."Processes" ("ProcessGuid","ProcessId","Image") ' \
                                 "VALUES ('{}',{},'{}') ON CONFLICT DO NOTHING;".format(
                                    event["Event"]["EventData"]["TargetProcessGuid"],
        ...
        if event["Event"]["System"]["EventID"] == 10:
            ...
            try:  # File - Actions
                dlls = str(event["Event"]["EventData"]["CallTrace"]).split("|")
                for dll in dlls:
                    path = dll.split("+")
                    if (path[0]).lower() not in (event["Event"]["EventData"]["SourceImage"]).lower():
            ...
            try:  # Thread
                ...
                query_thread = 'INSERT INTO public."Threads" ("ThreadId","ThreadNId","ProcessGuid")' \
                               " VALUES ('{}','{}','{}') ON CONFLICT DO NOTHING;".format(
                                thread_key,
                                event["Event"]["EventData"]["SourceThreadId"],
                                event["Event"]["EventData"]["SourceProcessGUID"])
                cursor.execute(query_thread)
            except Exception as e:
        ...
        if event["Event"]["System"]["EventID"] == 11:  # Create File
            ...
            if event["Event"]["EventData"]["TargetFilename"]:
                event["Event"]["EventData"]["TargetFilename"] = str(
                    event["Event"]["EventData"]["TargetFilename"]).replace("'", "\"")
