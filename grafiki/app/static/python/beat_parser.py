        ...
        if "log_name" in event:
            if "Sysmon" in event["log_name"]:
                ...
                if event["event_id"] == 1:
                    ...
                    try:
                        if event["event_data"]["Image"] not in files_inserted:
                            if "OriginalFileName" in event["event_data"]:
                                original = ",'" + str(event["event_data"]["OriginalFileName"]) + "'"
                            ...
                            query_file = 'INSERT INTO public."Files" ("Filename","FileVersion","Description","Product","Company"' \
                                         ...
                                         "VALUES ('{}','{}','{}','{}','{}' {});".format(
                                            key,
                                            event["event_data"]["Image"], event["event_data"]["FileVersion"],
                    ...
                    try:
                        query_pprocess = 'INSERT INTO public."Processes" ("ProcessGuid","ProcessId","Image", "ComputerName") ' \
                                         "VALUES ('{}',{},'{}','{}') ON CONFLICT DO NOTHING;".format(
                                            event["event_data"]["ParentProcessGuid"],
                    ...
                    try:
                        query_user = 'INSERT INTO public."Users" ("LogonGuid","Name","LogonId","TerminalSessionId") VALUES' \
                                     " ('{}','{}','{}',{}) ON CONFLICT DO NOTHING;".format(event["event_data"]["LogonGuid"],
                                                                                           event["event_data"]["User"],
                ...
                if event["event_id"] == 7:
                    ...
                    if event["event_data"]["ImageLoaded"]:
                        event["event_data"]["ImageLoaded"] = str(
                            event["event_data"]["ImageLoaded"]).replace("'", "\"")
                ...
                if event["event_id"] == 8:
                    ...
                    try:  # Target Process
                        query_tprocess = 'INSERT INTO public."Processes" ("ProcessGuid","ProcessId","Image", "ComputerName") ' \
                                         "VALUES ('{}',{},'{}','{}') ON CONFLICT DO NOTHING;".format(
                                            event["event_data"]["TargetProcessGuid"],
                                            event["event_data"]["TargetProcessId"],
                                            event["event_data"]["TargetImage"],
                                            event["computer_name"])
                ...
                if event["event_id"] == 11:  # Create File
                    ...
                    if event["event_data"]["TargetFilename"]:
                        event["event_data"]["TargetFilename"] = str(
                            event["event_data"]["TargetFilename"]).replace("'", "\"")

                    try:  # Process
                        insert_process(cursor, event)
                    except Exception as e:
                        print("Error query_sprocess 11: " + str(e) + " Event: " + str(event))

                    try:  # File
                        query_file = 'INSERT INTO public."Files" ("Filename","CreationUtcTime") ' \
                                     "VALUES ('{}','{}') ON CONFLICT DO NOTHING;".format(
                                        event["event_data"]["TargetFilename"],
                ...
                if event["event_id"] == 12 or event["event_id"] == 13 \
                    ...
                    try:  # Process
                        ...
                        print("Error query_process 12-13-14: " + str(e) + " Event: " + str(event))
                ...
                if event["event_id"] == 17 or event["event_id"] == 18:
                    try:
                        ...
                        print("Error insert_process 17-18: " + str(e) + " Event: " + str(event))
                ...
                if event["event_id"] == 22:
                    try:
                        ...
                        print("Error insert_process 22: " + str(e) + " Event: " + str(event))
